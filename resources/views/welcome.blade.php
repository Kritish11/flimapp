                        ...
                        const Category = {
                            ...
                            async handleSubmit(event) {
                                ...
                                try {
                                    ...
                                    const url = isUpdate ? `/categories/${id}` : '/categories';
                                    ...
                                    if (data.success) {
                                        ...
                                        if (isUpdate) {
                                            this.updateInGrid(data.category);
                                        } else {
                                            this.addToGrid(data.category);
                                        }
                                    } else {
                            ...
                            showToast(message, type = 'success') {
                                ...
                                toast.classList.remove('bg-green-100', 'border-green-400', 'text-green-700');

                                if (type === 'error') {
                                    toast.classList.add('bg-red-100', 'border-red-400', 'text-red-700');
                                } else {
                                    toast.classList.add('bg-green-100', 'border-green-400', 'text-green-700');
                                }

                                setTimeout(() => toast.classList.add('hidden'), 3000);
                            },

                            updateInGrid(category) {
                            ...
                            addToGrid(category) {
                                const html = this.getCategoryHTML(category);
                                ...
                                return `
                                    <div class="bg-white rounded-xl shadow-md p-6 hover:shadow-lg transition-shadow" id="category-${category.id}">
                                        <div class="flex justify-between items-start mb-4">
                                            ...
                                            <div class="flex gap-2">
                                                <button onclick="Category.openModal(${JSON.stringify(category)})" class="p-2 hover:bg-gray-100 rounded-lg">✏️</button>
                                                <button onclick="Category.confirmDelete(${category.id}, '${category.name}')" class="p-2 hover:bg-gray-100 rounded-lg">🗑️</button>
                                            </div>
                                        </div>
                                        <div class="bg-gray-50 rounded-lg p-3">
